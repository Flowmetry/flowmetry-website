...
interface GlareCardProps {
  ...
  className?: string;
  /** Fixed tilt preset for mobile (no interaction). Each card gets a unique look. */
  mobilePreset?: { rx: number; ry: number; mx: number; my: number };
}
...
export const GlareCard = ({
  ...
  // Apply mobile preset on mount and when isMobile changes
  useEffect(() => {
    if (isMobile && mobilePreset && refElement.current) {
      const el = refElement.current;
      el.style.setProperty("--r-x", `${mobilePreset.rx}deg`);
      el.style.setProperty("--r-y", `${mobilePreset.ry}deg`);
      el.style.setProperty("--m-x", `${mobilePreset.mx}%`);
      el.style.setProperty("--m-y", `${mobilePreset.my}%`);
      el.style.setProperty("--bg-x", `${50 + mobilePreset.mx / 4 - 12.5}%`);
      el.style.setProperty("--bg-y", `${50 + mobilePreset.my / 3 - 16.67}%`);
      el.style.setProperty("--opacity", "0");
      el.style.setProperty("--duration", "0s");
    }
    if (!isMobile && refElement.current) {
      const el = refElement.current;
  ...
  // On mobile: no pointer handlers, static preset
  ...
  return (
    <div
      ...
      className="relative isolate [contain:layout_style] [perspective:600px] transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-transform w-full sm:w-[320px] sm:[aspect-ratio:17/21]"
      ref={refElement}
      {...pointerHandlers}
    >
